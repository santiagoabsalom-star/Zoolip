...
public class AtencionView extends FormLayout {
    ...
    private void save() {
        ...
        if (fechaInicio.getValue() != null)
            toSave.setFecha_inicio(LocalDateTime.of(fechaInicio.getValue(), LocalTime.NOON));
        if (fechaFinal.getValue() != null)
            toSave.setFecha_final(LocalDateTime.of(fechaFinal.getValue(), LocalTime.NOON));
        ...
        try {
            ...
            if (res.getStatus().equals("error")) Notification.show("Error: " + res.getMessage());
                ...
                Notification.show("Atención guardada");
                refreshGrid();
                edit(null);
            }
        } catch (Exception ex) {
            Notification.show("Error al guardar: " + ex.getMessage());
            log.error("Save atencion failed", ex);
        }
    }
    ...
    private void confirmDelete() {
        ...
        Button yes = new Button("Eliminar", ev -> {
            try {
                atencionService.eliminar(current);
                Notification.show("Atención eliminada");
                refreshGrid();
                edit(null);
            } catch (Exception ex) {
    ...
    private void refreshGrid() {
        try {
            var items = atencionService.obtenerTodas();
            if (items != null) grid.setItems(items);
        } catch (Exception e) {
            log.error("Failed to refresh atenciones", e);
        }
    }
}
